...
export class PollDetails extends Component<DetailsProps, DetailsState> {
  constructor(props: DetailsProps) {
    super(props);
    this.state = {now: Date.now(), poll: undefined, voterName: "", chosenOption: "",  error: "", voted: false};
  }
      ...
      for (const option of poll.optionObj) {
        ...
        results.push(
          <li key={option.optionName}>
            {`${percentage.toFixed(2)}%`} - {option.optionName}
          </li>
        );
      }
      return (
      ...
      optionShow.push(
        <ul key={option}>
         ...
         if (this.state.voted) {
          voteMsg = (
            <p>
              Recorded vote of "{this.state.voterName}" as "{this.state.chosenOption}"
            </p>
          );
         }
...
doVoteClick = (_: MouseEvent<HTMLButtonElement>): void => {
  ...
  const total = this.state.poll.totalVotes + 1;

  const args = {pollName: this.props.name, voterName: this.state.voterName, totalVotes: total,
